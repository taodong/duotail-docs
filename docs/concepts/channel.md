...
An **Identity Channel** is assigned to users whose real-world identities have been successfully verified. Unlike alias channels, an Identity Channel email address has no prefix (e.g., `isaac.swordblessing@duotail.com`). This serves as a "verified badge," signaling to recipients that the sender's identity is authentic and confirmed.